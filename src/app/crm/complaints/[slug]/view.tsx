    ...
    const worksheet = XLSX.utils.json_to_sheet([
      {
        ...
        "Purchase Date": complaint.p_date ? formatDate(complaint.p_date) : "N/A",
        "Completion Date": complaint.complete_date ? formatDate(complaint.complete_date) : "N/A",
      ...
      <Card className="p-4 bg-white border rounded-lg shadow-sm">
        ...
        <div className="flex flex-wrap items-center justify-between gap-4 mb-6">
          <div className="flex flex-col gap-2 sm:flex-row sm:items-center">
            ...
            <Badge className={`px-3 py-1 text-sm font-medium ${getStatusColor(complaint.status)}`}>
              {complaint.status}
            </Badge>
          </div>
        ...
        <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-3">
          {/* Customer Info */}
          <div className="p-4 ">
            <h3 className="flex items-center gap-2 mb-4 text-base font-semibold text-primary">
            ...
            <div className="space-y-3">
              ...
              {complaint.applicant_email && (
                <div className="grid grid-cols-[100px_1fr] gap-2 text-sm">
                  ...
                  <span className="font-medium break-all">{complaint.applicant_email}</span>
                </div>
              )}
              <div className="grid grid-cols-[100px_1fr] gap-2 text-sm">
                ...
                <div>
                  ...
                  <div className="font-medium">{complaint.applicant_whatsapp}</div>
                </div>
              </div>
              <div className="grid grid-cols-[100px_1fr] gap-2 text-sm">
                <span className="text-gray-500">Address:</span>
                <span className="font-medium">{complaint.applicant_adress}</span>
              </div>
            </div>
          </div>
          ...
          <div className="p-4 ">
            ...
            <div className="space-y-3">
              <div className="grid grid-cols-[100px_1fr] gap-2 text-sm">
                ...
                <span className="font-medium">{complaint.product} {complaint.model}</span>
              </div>
              {complaint.serial_number_ind && (
                <div className="grid grid-cols-[100px_1fr] gap-2 text-sm">
                  <span className="text-gray-500">SN (IND):</span>
                  <span className="font-medium">{complaint.serial_number_ind}</span>
                </div>
              )}
              {complaint.serial_number_oud && (
                <div className="grid grid-cols-[100px_1fr] gap-2 text-sm">
                  <span className="text-gray-500">SN (OUD):</span>
                  <span className="font-medium">{complaint.serial_number_oud}</span>
                </div>
              )}
              <div className="grid grid-cols-[100px_1fr] gap-2 text-sm">
                <span className="text-gray-500">Technician:</span>
                <span className="font-medium">{complaint.technician || "N/A"}</span>
              </div>
              {complaint.helper && (
                <div className="grid grid-cols-[100px_1fr] gap-2 text-sm">
            ...
            <div className="space-y-3">
              ...
              {complaint.p_date && (
                <div className="grid grid-cols-[100px_1fr] gap-2 text-sm">
                  ...
                  <span className="font-medium">{formatDate(complaint.p_date)}</span>
                </div>
              )}
              <div className="grid grid-cols-[100px_1fr] gap-2 text-sm">
                <span className="text-gray-500">Description:</span>
                <span className="font-medium whitespace-pre-wrap">{complaint.description}</span>
              </div>
            </div>
          </div>
        </div>
          ...
          <div className="mt-6">
            ...
            <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
              {files.map((file: any, index: number) => {
                ...
                return (
                  <Card key={index} className="overflow-hidden bg-white border">
                    {isImage ? (
                      <a href={getImageUrl(file.document_path)} target="_blank" rel="noopener noreferrer" className="block">
                        ...
                        <div className="p-3">
                          <p className="mb-1 font-medium truncate">{file.file_name}</p>
