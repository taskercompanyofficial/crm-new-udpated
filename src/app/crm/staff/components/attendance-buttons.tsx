...
export function AttendanceButtons({ userId, date, checkIn, checkOut }: AttendanceButtonsProps) {
    ...
    return (
        <div className="flex gap-2">
            <Button
                ...
                disabled={isLoading}
            >
                {checkIn ? <LogOut className="w-4 h-4" /> : <LogIn className="w-4 h-4" />}
                {isLoading ? "Loading..." : (checkIn ? "Check Out" : "Check In")}
            </Button>
        </div>
    );
}